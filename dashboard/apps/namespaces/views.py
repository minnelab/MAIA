...
def namespace_view(request,namespace_id):
    ...
    if namespace_id.lower().replace("_","-") not in namespaces and not request.user.is_superuser:
        ...
        if request.user.is_superuser:
            is_admin = True
        maia_workspace_apps, remote_desktop_dict, ssh_ports, monai_models, orthanc_list, deployed_clusters, nvflare_dashboards = get_namespace_details(settings, id_token, namespace_id, user_id, is_admin=is_admin)
