...
class TestMAIACore:
    ...
    def test_minio_tenant(self):
        ...
        dict_data = {"name": "test.txt", "content": "Hello, World!"}
        minio_client.put_object(
            "mlflow",
            "test.json",
            io.BytesIO(json.dumps(dict_data).encode()),
            length=len(json.dumps(dict_data)),
    ...
    def test_grafana(self):
        ...
        get_api_keys_command = [
            ...
            "-k",
            "-u",
            f"{os.environ.get('GRAFANA_ADMIN_USER')}:{os.environ.get('GRAFANA_ADMIN_PASSWORD')}",
        ...
        post_dashboard_command = [
            ...
            "-d",
            json.dumps(post_dashboard_payload),
        ]
        subprocess.run(
            post_dashboard_command,
            stdout=subprocess.PIPE,
        ...
        search_result = subprocess.run(search_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        ...
        for item in search_json:
            if item["title"] == dashboard_title:
                ...
                dashboard_payload["time"] = {"from": "now-1h", "to": "now"}
    ...
    def test_metrics_server(self):
        ...
        metrics = api.list_cluster_custom_object(group="metrics.k8s.io", version="v1beta1", plural="pods")
